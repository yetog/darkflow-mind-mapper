...
import ConversationTypeInsight from '@/components/common/ConversationTypeInsight';
...
const Header = ({
  ...
  const currentType = conversationType 
    ? CONVERSATION_TYPES.find(t => t.id === conversationType) 
    : null;
    ...
    <header className="relative z-40 flex items-center justify-between h-14 px-6 border-b border-border bg-card/50 backdrop-blur-sm">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-foreground">{title || 'Untitled'}</h2>
            {currentType && (
      ...
      <div className="flex items-center gap-3">
        {/* Conversation Type Selector - only show when type is provided */}
        {conversationType && onTypeChange && (
      ...
      {conversationType && <ConversationTypeInsight type={conversationType} />}
    </header>
  );
};
